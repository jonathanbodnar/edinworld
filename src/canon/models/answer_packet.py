...
class AnswerPacket(UUIDPrimaryKeyMixin, Base):
    ...
    sources: Mapped[list[AnswerPacketSource]] = relationship(back_populates="answer_packet")
    contexts: Mapped[list[AnswerPacketContext]] = relationship(back_populates="answer_packet")


class AnswerPacketSource(UUIDPrimaryKeyMixin, Base):
    __tablename__ = "answer_packet_sources"

    answer_packet_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False, index=True)
